...
void ahe_server(const ui32 numClients = 1, const bool verbose = false) {
    ...
    // Step 1:
    //  Generate and send the common random seed
    //  Receive public keys from clients
    //  Add the public keys together to create the multiparty key.
...
void ahe_client(const string name = "Client", const bool manualEntry = false, const bool verbose = false) {
    ...
    data[0] = 10;
    if (manualEntry) {
        std::cout << "Please enter your data value: ";
        std::cin >> data[0];
    }
    std::cout << name << " data: " << data[0] << std::endl;
...
int main(int argc, char** argv) {
	...
	if(argc >= 2 && argc <= 4)
	{
        ...
        if (argc == 2) role ? ahe_client(name) : ahe_server();
            ...
            if (argc == 3) role ? ahe_client(name, numClients!=0) : ahe_server(numClients);
            else {
                int verbose = atoi(argv[3]);
                if (argc == 4) role ? ahe_client(name, numClients!=0, verbose!=0) : ahe_server(numClients, verbose!=0);
